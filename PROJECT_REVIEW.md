...
11. **Excel Project Recognition (`المشروع`), Multi-Project Order Separation & Display**:
    - **Adaptive Excel Project Column Parsing:** Added intelligent detection for the `"المشروع"` (Project / Client) column in incoming Excel order files. Employs section-based inheritance to properly assign project names (e.g. `نادك`) even when project names are defined once for a city block or on merged rows.
    - **Separate Orders for Distinct Projects:** If an uploaded Excel file contains 2 or more projects (e.g. `نادك` and `المراعي` or Dammam branches), the system automatically defaults to `PER_PROJECT` mode and generates completely independent orders for each project instead of merging them into one.
    - **Executive Project Naming in City & Recipient Column:** Instead of defaulting to `مشروع [City]` (e.g. `مشروع الاحساء`), when a project name exists in the Excel file, the order title dynamically displays **`مشروع نادك`** (e.g. `مشروع نادك (بلال محمد / طلال طلعت)`) accompanied by the regional city badge (`🏙️ الاحساء`).
